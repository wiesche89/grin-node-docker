// qrc:/qml/QRGen.js
// Minimal QML-friendly wrapper around Kazuhiko Arase QR generator.
// Exposes a stable namespace for QML: QR.create(...), QR.setUtf8()
...
var qrcode = function() {
  ...
  var qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    var createBytes = function(buffer, rsBlocks) {
      ...
      for (var a = 0; a < maxDcCount; a += 1) {
        for (var b = 0; b < rsBlocks.length; b += 1) {
          if (a < dcdata[b].length) {
            data[index] = dcdata[b][a];
            index += 1;
          }
        }
      }

      for (var c = 0; c < maxEcCount; c += 1) {
        for (var d = 0; d < rsBlocks.length; d += 1) {
          if (c < ecdata[d].length) {
            data[index] = ecdata[d][c];
            index += 1;
          }
        }
      }
  ...
  var QRUtil = (function() {
    ...
    _this.getLostPoint = function(qrcodeObj) {

      var moduleCount = qrcodeObj.getModuleCount();
      var lostPoint = 0;
      ...
      for (var row = 0; row < moduleCount; row += 1) {
        for (var col = 0; col < moduleCount; col += 1) {
          ...
          var sameCount = 0;
          var dark = qrcodeObj.isDark(row, col);
          ...
          for (var r = -1; r <= 1; r += 1) {
            ...
            for (var c = -1; c <= 1; c += 1) {
              ...
              if (r == 0 && c == 0) continue;
              if (dark == qrcodeObj.isDark(row + r, col + c)) sameCount += 1;
            }
          }
      ...
      // LEVEL2
      for (var r2 = 0; r2 < moduleCount - 1; r2 += 1) {
        for (var c2 = 0; c2 < moduleCount - 1; c2 += 1) {
          var count = 0;
          if (qrcodeObj.isDark(r2, c2)) count += 1;
          if (qrcodeObj.isDark(r2 + 1, c2)) count += 1;
          if (qrcodeObj.isDark(r2, c2 + 1)) count += 1;
          if (qrcodeObj.isDark(r2 + 1, c2 + 1)) count += 1;
          if (count == 0 || count == 4) lostPoint += 3;
        }
      }

      // LEVEL3
      for (var r3 = 0; r3 < moduleCount; r3 += 1) {
        for (var c3 = 0; c3 < moduleCount - 6; c3 += 1) {
          if (qrcodeObj.isDark(r3, c3)
              && !qrcodeObj.isDark(r3, c3 + 1)
              &&  qrcodeObj.isDark(r3, c3 + 2)
              &&  qrcodeObj.isDark(r3, c3 + 3)
              &&  qrcodeObj.isDark(r3, c3 + 4)
              && !qrcodeObj.isDark(r3, c3 + 5)
              &&  qrcodeObj.isDark(r3, c3 + 6)) {
            lostPoint += 40;
          }
        }
      }

      for (var c4 = 0; c4 < moduleCount; c4 += 1) {
        for (var r4 = 0; r4 < moduleCount - 6; r4 += 1) {
          if (qrcodeObj.isDark(r4, c4)
              && !qrcodeObj.isDark(r4 + 1, c4)
              &&  qrcodeObj.isDark(r4 + 2, c4)
              &&  qrcodeObj.isDark(r4 + 3, c4)
              &&  qrcodeObj.isDark(r4 + 4, c4)
              && !qrcodeObj.isDark(r4 + 5, c4)
              &&  qrcodeObj.isDark(r4 + 6, c4)) {
            lostPoint += 40;
          }
        }
      }
      ...
      // LEVEL4
      var darkCount = 0;
      for (var c5 = 0; c5 < moduleCount; c5 += 1) {
        for (var r5 = 0; r5 < moduleCount; r5 += 1) {
          if (qrcodeObj.isDark(r5, c5)) darkCount += 1;
        }
      }
  ...
  function qrPolynomial(num, shift) {
    ...
    var _num = (function() {
      ...
      var nn = new Array(num.length - offset + shift);
    ...
    _this.multiply = function(e) {
      ...
      for (var a = 0; a < _this.getLength(); a += 1) {
        for (var b = 0; b < e.getLength(); b += 1) {
          out[a + b] ^= QRMath.gexp(QRMath.glog(_this.getAt(a)) + QRMath.glog(e.getAt(b)));
        }
      }
  ...
  var QRRSBlock = (function() {
    ...
    // RS block table (same as original)
    var RS_BLOCK_TABLE = [
  ...
  var qrNumber = function(data) {
    ...
    return {
      ...
      write: function(buffer) {
        ...
        if (i < _data.length) {
          if (_data.length - i == 1) buffer.put(strToNum(_data.substring(i, i + 1)), 4);
          else if (_data.length - i == 2) buffer.put(strToNum(_data.substring(i, i + 2)), 7);
        }
      }
    };
  };
  ...
  var qrAlphaNum = function(data) {
    ...
    return {
      ...
      write: function(buffer) {
        ...
        if (i < _data.length) buffer.put(getCode(_data.charAt(i)), 6);
      }
    };
  };
  ...
  var qrKanji = function(data) {
    ...
    return {
      getMode: function() { return _mode; },
      getLength: function() { return ~~(_bytes.length / 2); },
      write: function(buffer) {
        var i = 0;
        while (i + 1 < _bytes.length) {
          var c = ((0xff & _bytes[i]) << 8) | (0xff & _bytes[i + 1]);
        ...
        if (i < _bytes.length) throw 'illegal char at ' + (i + 1);
      }
    };
  };
...
// UTF-8 support for QML (no AMD/CommonJS footer)
...
// QML exports
function setUtf8Enabled(enabled) {
  if (enabled) qrcode.stringToBytes = qrcode.stringToBytesFuncs["UTF-8"];
  else qrcode.stringToBytes = qrcode.stringToBytesFuncs["default"];
}

function create(typeNumber, errorCorrectionLevel) {
